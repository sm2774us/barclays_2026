...
> "At my last seat within our systematic macro pod, our execution and financing desks needed a way to flag anomalous, toxic, or heavily mispriced liquidity quotes across fragmented macro venues (FX ECNs and Fixed Income/Equity order books) before our routing algorithms hit the book.
> **Situation:** Post-trade manual review and basic regression thresholds only caught about 60% of these execution anomalies, meaning we were constantly absorbing adverse selection costs after the fact.
> **Task:** Build and deploy a real-time, automated pre-trade filtering layer that could score incoming quotes instantly without introducing latency bottlenecks into our execution pipeline.
> **Action:** I evaluated the trade-offs between deep learning and traditional ML, choosing to start with the simplest defensible baseline—a gradient-boosted classifier built on feature-engineered z-scores of quote-vs-peer-venue books. I deliberately bypassed a neural net on day one because false positives would break trader trust and stall order routing; I needed absolute explainability from the start. I validated this model against 18 months of high-frequency tick history, worked closely with our platform engineering team to wrap it as a low-latency scoring service in production, and instrumented it with drift monitors on the input market distributions.
> **Result:** The model successfully captured 91% of toxic liquidity anomalies while maintaining a sub-3% false-positive rate, directly protecting our implementation shortfall metrics.
> **The lesson I carry into this Barclays Liquid Financing role:** Adopting that start-up mindset means you ship the most robust, explainable baseline that clears the risk and latency bar first. Once you secure that production floor, you earn the right to iterate with advanced sequence modeling—like an LSTM or a Transformer architecture—where the residual error justifies the engineering complexity."
